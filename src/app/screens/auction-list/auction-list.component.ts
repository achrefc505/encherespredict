      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px;">
        <div>
          ...
          <div style="font-size:13px;color:var(--text-3);margin-top:5px;">
            ...
            <span style="color:#10B981;">mis à jour il y a 12 min</span>
          </div>
        </div>
            ...
            <tbody>
              @for (a of filtered(); track a.id; let last = $last) {
                <tr (click)="goDetail(a)"
                  ...
                  <td style="padding:13px 16px;color:var(--text-3);white-space:nowrap;">{{ fmtDate(a.date) }}</td>
                  <td style="padding:13px 16px;">
          ...
          @for (a of filtered(); track a.id) {
            <div (click)="goDetail(a)" class="auction-card" style="background:var(--surface-2);border:1px solid var(--border);border-radius:12px;overflow:hidden;cursor:pointer;transition:all 0.15s ease;">
              ...
              <div style="padding:16px;">
                ...
                <div style="font-size:11px;color:var(--text-3);margin-bottom:14px;">{{ a.tribunal }} · {{ fmtDate(a.date) }}</div>
...
export class AuctionListComponent {
  ...
  cities = [...new Set(MOCK_AUCTIONS.map(a => a.city))];
  types  = [...new Set(MOCK_AUCTIONS.map(a => a.type))];

  filtered = computed(() => {
    const c = this.city(), t = this.type(), b = this.badge(), s = this.sort();
    let list = MOCK_AUCTIONS.filter(a =>
      (!c || a.city === c) &&
      (!t || a.type === t) &&
      (!b || a.badge === b)
    );
    ...
    if (s === 'date')  list = [...list].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  pct(a: Auction) { return a.startPrice ? Math.round(((a.aiEstimate - a.startPrice) / a.startPrice) * 100) : 0; }
